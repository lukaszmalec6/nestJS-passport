  ...
  private init(): void {
    use(JWTStrategySymbols.register, new Strategy({
      ...
      try {
        ...
        const hash: string = this.generateHashedPassword(password, this.config.auth.salt);
        const user: User = await this.userService.create({firstName, lastName, email, password: hash});
